...
public class BoardDAO_place {
	...
	public void insertBoard(BoardBean bb) {
		...
		try {
			...
			// 3. sql 쿼리 & pstmt 객체
			// 게시판 글번호(num) 계산
			sql = "select max(num) from gb_board_place";
			...
			// 글번호 + 글정보 => 글쓰기
			// 3. sql 쿼리 & pstmt 객체
			sql = "insert into gb_board_place (num,name,pass,subject,content,"
	...
	public int getBoardCount() {
		...
		try {
			...
			sql = "select count(*) from gb_board_place";
	...
	public ArrayList getBoardList() {
		...
		try {
			...
			// limit 시작행-1, 개수
			sql = "select * from gb_board_place";
...
//					+ " limit 0,5";
	...
	public ArrayList getBoardList(int startRow, int pageSize) {
		...
		try {
			...
			// re_ref(num) 기준으로 정렬 내림차순, re_seq 오름차순 정렬
			// limit 시작행-1, 개수
			sql = "select * from gb_board_place" + " order by re_ref desc, re_seq asc" + " limit ?,?";
	...
	public void updateReadCount(int num) {
		...
		try {
			...
			// 3. sql 쿼리 & pstmt 객체
			// 기존의 조회수를 1 증가
			sql = "update gb_board_place set readcount=readcount+1 where num=?";
	...
	public BoardBean getBoard(int num) {
		...
		try {
			...
			// 3. sql 구문 & pstmt 객체
			sql = "select * from gb_board_place where num=?";
	...
	public int updateBoard(BoardBean bb){
		...
		// 3. sql 작성 (select - 글이 있는지 판단) & pstmt 객체
			sql = "select pass from gb_board_place where num=?";
		...
		// 5. 데이터 처리
			if(rs.next()){
				if(bb.getPass().equals(rs.getString("pass"))){
					...
					sql = "update gb_board_place set name=?,subject=?, "
							+ "content=? where num=?";
		...
		// 5. 데이터 처리
			if (rs.next()){
				if(bb.getPass().equals(rs.getString("pass"))){
					...
					sql = "delete from gb_board_place where num=?";
...
///////////////////////////////////////////////////////////////////////
			...
			sql = "update gb_board_place set re_seq=re_seq+1 "
